...
export function BasketCard(product: IPropsProduct) {
  return (
    <div className="good-card" key={product.product.id}>
      ...
      (<p> <span className="cart__card-price">${product.product.price}</span></p>)
      :( <p> <span className="cart__card-price">${product.product.price}</span> ( <span className="cart__card-price-old">${product.product.oldprice}</span>)</p>)
      }
      <input
